...
namespace XmlLibrary
{
  ...
  void Parser::ParseMiscs(void)
  {
    for(;;)
    {
      ...
      std::unique_ptr<Comment> pElem(ParseComment());
      if(NULL == pElem.get())
        break;
    }
  }
  ...
  bool Parser::ParseAttribute(Tag* pElem)
  {
    ...
    std::unique_ptr<Attribute> pAttrib(new Attribute(strName, strValue));
  ...
  Tag* Parser::ParseTagBegining(void)
  {
    ...
    std::unique_ptr<Tag> pElem(new Tag(strName));
  ...
  {
    std::unique_ptr<Comment> pComment(ParseComment());
    ...
    std::unique_ptr<Tag> pTag(ParseElement());
    if(NULL != pTag.get())
    {
  ...
  {
    std::unique_ptr<Tag> pTag(ParseTagBegining());
    if(pTag.get() == NULL)
      return(NULL);
  ...
  Element* Parser::ParseDocument(void)
  {
    ...
    std::unique_ptr<Element> pElem(ParseElement());
    if(NULL == pElem.get())
      RaiseSyntaxError();
  ...
  Element& Parser::Parse(std::istream* pInputStream)
  {
    ...
    std::unique_ptr<Element> temp(ParseDocument());
    m_pRootElem=temp;
